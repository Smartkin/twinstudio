...
static const uint32_t maxStructFields = 1024;
static const uint32_t maxEnums = 256;
static const uint32_t maxAttributeArgs = 16;
static const uint32_t maxAstNodes = UINT16_MAX;
static const size_t maxResourceFileSize = 2 * 1024 * 1024; // max 2MB size of a single output file
...
static TwinRes_ParserNode LiteralNode(TwinRes_GeneratorOutput* output, TwinRes_Lexer* lexer)
{
    ...
    switch (lexer->curTok.tokenType) {
        ...
        case TwinRes_TokOpenBracket:
        {
            ...
            int32_t objectDefinitionDepth = 0;
            while (lexer->curTok.tokenType != TwinRes_TokCloseBracket || objectDefinitionDepth > 0)
            {
                TwinRes_LexerNextToken(lexer);
                if (lexer->curTok.tokenType == TwinRes_TokOpenBracket)
                {
                    objectDefinitionDepth++;
                }
                else if (lexer->curTok.tokenType == TwinRes_TokCloseBracket)
                {
                    objectDefinitionDepth--;
                }
            }
...
static bool HandleFieldNoLength(TwinRes_StructFieldGeneratorOptions* options, TwinRes_AttributeArgumentsList* arguments)
{
    ...
    options->noLength.argumentLength = arguments->length;
    options->noLength.arguments = arguments->arguments;
    return true;
}
...
static bool HandleFieldCustomBin(TwinRes_StructFieldGeneratorOptions* options, TwinRes_AttributeArgumentsList* arguments)
{
    if (arguments->length < 2)
    {
        return false;
    ...
    TwinStudio_VariantSetBool(&options->customBin.data, true);
    options->customBin.argumentLength = arguments->length;
    options->customBin.arguments = arguments->arguments;
    return true;
}
...
static int GenerateStructField(TwinRes_ParserNode* node, TwinRes_GeneratedFile* genFile, char* buffer, bool selfTypeReference)
{
    assert(node->type == TwinRes_NodeFieldDefinition);

    static const char structSelfReferenceFormat[] = "   struct "TS_VIEW_FORMAT"* "TS_VIEW_FORMAT";\n";
    ...
    if (selfTypeReference)
    {
        return WriteFile(genFile, buffer, structSelfReferenceFormat, TS_VIEW_ARG(fieldTypeName), TS_VIEW_ARG(fieldName));
    }
...
static int GenerateStructFieldBinarySerialization(TwinRes_ParserNode* node, TwinRes_GeneratedFile* genFile, char* buffer)
{
    ...
    const bool customBinSerial = TS_VARIANT_GET(bool, structField->options.customBin.data);
    char serialFuncName[2048];
    if (valueType == TwinStudio_VariantObject && customBinSerial)
    {
        snprintf(serialFuncName, 2048, TS_VIEW_FORMAT, TS_VIEW_ARG(TS_VARIANT_GET(TwinStudio_StringView, structField->options.customBin.arguments[0])));
    }
    else
    {
        snprintf(serialFuncName, 2048, TS_VIEW_FORMAT"BinSerialize", TS_VIEW_ARG(structField->type));
    }
    switch (valueType)
    {
        ...
        case TwinStudio_VariantObject:
            if (!isLengthDefined)
            {
                fieldBodyWritten = WriteFile(genFile, buffer, "%s%s(&source->"TS_VIEW_FORMAT"%s, serializer, arena, sizeof("TS_VIEW_FORMAT"), source);\n", extraSpaces, serialFuncName, TS_VIEW_ARG(structField->name), indexWriting, TS_VIEW_ARG(structField->type));
            }
            else
            {
                fieldBodyWritten = WriteFile(genFile, buffer, "%s%s(&source->"TS_VIEW_FORMAT"%s, serializer, arena, %s, source);\n", extraSpaces, serialFuncName, TS_VIEW_ARG(structField->name), indexWriting, sizeFormat);
            }
            break;
        default:
            break;
...
static int GenerateStructFieldsBinaryDeserialization(const TwinRes_ParserStructDefinition* structDef, TwinRes_GeneratedFile* genFile, char* buffer)
{
    ...
    for (uint32_t i = 0; i < fieldsAmount; ++i)
    {
        ...
        const bool customBinSerial = TS_VARIANT_GET(bool, structField->options.customBin.data);
        char deserialFuncName[2048];
        if (valueType == TwinStudio_VariantObject && customBinSerial)
        {
            snprintf(deserialFuncName, 2048, TS_VIEW_FORMAT, TS_VIEW_ARG(TS_VARIANT_GET(TwinStudio_StringView, structField->options.customBin.arguments[1])));
        }
        else
        {
            snprintf(deserialFuncName, 2048, TS_VIEW_FORMAT"BinDeserialize", TS_VIEW_ARG(structField->type));
        }
        if (isArray)
        {
            if (valueType != TwinStudio_VariantObject)
        ...
        if (isArray)
        {
            if (isInfArray)
            {
                char* readCondition = "TwinStudio_BinGetStreamPosition(deserializer) < TwinStudio_BinGetStreamLength(deserializer)";
    ...
    for (uint32_t i = 0; i < fieldsAmount; ++i)
    {
        ...
        switch (valueType)
        {
            ...
            case TwinStudio_VariantString:
                if (!isArray)
                {
                    fieldBodyWritten = WriteFile(genFile, buffer, "   char* dynCStr"TS_VIEW_FORMAT" = TwinStudio_GetCStringDynamic(&source->"TS_VIEW_FORMAT");\n   cJSON_AddStringToObject(rootObject, \""TS_VIEW_FORMAT"\", dynCStr"TS_VIEW_FORMAT");\n   TWIN_FREE(dynCStr"TS_VIEW_FORMAT")", TS_VIEW_ARG(structField->name), TS_VIEW_ARG(structField->name), TS_VIEW_ARG(structField->name), TS_VIEW_ARG(structField->name), TS_VIEW_ARG(structField->name));
                }
                else 
                {
                    fieldBodyWritten = WriteFile(genFile, buffer, "   char* dynCStr"TS_VIEW_FORMAT" = TwinStudio_GetCStringDynamic(&source->"TS_VIEW_FORMAT"[i]);\n   cJSON_AddItemToArray("TS_VIEW_FORMAT"Json, cJSON_CreateString(dynCStr"TS_VIEW_FORMAT"));\n   TWIN_FREE(dynCStr"TS_VIEW_FORMAT"", TS_VIEW_ARG(structField->name), TS_VIEW_ARG(structField->name), TS_VIEW_ARG(structField->name), TS_VIEW_ARG(structField->name), TS_VIEW_ARG(structField->name));
                }
                break;
            case TwinStudio_VariantObject:
                if (!isArray)
...
static int GenerateStructDefinitions(TwinRes_ParserNode* node, TwinRes_GeneratedFile* genFile, char* buffer)
{
    ...
    if (!excludeJsonSerial)
    {
        ...
        if (!excludeDeserial)
        {
            ...
            int bodyEndWritten = WriteFile(genFile, buffer, "   }\n\n");
            if (bodyEndWritten == 0)
            {
                return 0;
            }
...
static int GenerateStructDeclarations(TwinRes_ParserNode* node, TwinRes_GeneratedFile* genFile, char* buffer, bool isInlineStruct)
{
    ...
    for (uint32_t i = 0; i < structDef->fields->length; ++i)
    {
        ...
        bool isSelfTypeReference = false;
        TwinRes_ParserStructFieldDefinition* fieldDef = node->data;
        if (strncmp(fieldDef->type.string, structDef->structName.string, TwinStudio_MaxInt(fieldDef->type.length, structDef->structName.length)) == 0)
        {
            isSelfTypeReference = true;
        }

        int fieldAmountWritten = GenerateStructField(node, genFile, buffer, isSelfTypeReference);
