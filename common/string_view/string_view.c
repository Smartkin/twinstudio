...
{
    TwinStudio_StringView* resultStr = rpmalloc(sizeof(TwinStudio_StringView));
...
TwinStudio_StringView* TwinStudio_ConvertStringToString(void* data)
{
    ...
    char* strCopy = rpmalloc(originalStr->length + 1);
...
TwinStudio_StringView* TwinStudio_ConvertIntToString(void* data)
{
    ...
    char* dynStr = rpmalloc(charsTotal + 1);
...
{
    const uint32_t stringSize = strlen(cStr);
    char* dynStr = rpmalloc(stringSize);
    memcpy(dynStr, cStr, stringSize);
...
{
    char* cString = rpmalloc(string->length + 1);
